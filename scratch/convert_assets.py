...
for size in [16, 32, 96]:
    fav.resize((size, size), Image.Resampling.LANCZOS).save(
        os.path.join(icons_dir, f'favicon-{size}x{size}.png'), 'PNG',
    )

# 3. Android launcher icons
for size in [36, 48, 72, 96, 144, 192, 256, 384, 512]:
    fav.resize((size, size), Image.Resampling.LANCZOS).save(
        os.path.join(icons_dir, f'android-chrome-{size}x{size}.png'), 'PNG',
    )

# 4. Apple Touch Icons
for size in [57, 60, 72, 76, 114, 120, 144, 152, 180]:
    fav.resize((size, size), Image.Resampling.LANCZOS).save(
        os.path.join(icons_dir, f'apple-touch-icon-{size}x{size}.png'), 'PNG',
    )
fav.resize((180, 180), Image.Resampling.LANCZOS).save(
    os.path.join(icons_dir, 'apple-touch-icon.png'), 'PNG',
)
fav.resize((180, 180), Image.Resampling.LANCZOS).save(
    os.path.join(icons_dir, 'apple-touch-icon-precomposed.png'), 'PNG',
)
